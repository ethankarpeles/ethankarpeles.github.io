    ...
    <>
      <article>
        ...
        <section>
          ...
          <h2>Triangles</h2>
          <p>
            Famously, the {PythagoreanTheorem} states that if{" "}
            <Latex math="a,b,c" /> are the side lengths of a right triangle,
            with <Latex math="c" /> being the hypotenuse, then these numbers
          ...
          <p>
            ...
            positive integers have no common factors, we call it a{" "}
            {PrimitivePythagoreanTriple}.
          </p>

          <p>Some examples of primitive Pythagorean Triples include:</p>
          <Latex
            math="\{3, 4, 5\}, \ \{5, 12, 13\}, \text{ and } \{8, 15, 17\}"
            center
          />

          <p>
            A non-example of a {PrimitivePythagoreanTriple} is{" "}
            <Latex math="\{6, 8, 10\}" />. Notice that these three numbers have
            a common factor of <Latex math="2" />, so it is not primitive.
            However, it is still a {PythagoreanTriple} since you could construct
            a right triangle with these side lengths.
          </p>
        </section>
        ...
        <section>
          ...
          <p>
            After seeing a few of these, it is natural to ask how many primitive
            Pythagorean Triples exist. Indeed, there is no obvious reason why
          ...
          <p>
            ...
            if we can pair them up one by one. For example, if everyone in a
        ...
        <section>
          ...
          <p>
            The unit circle is defined to be the circle of radius 1 centered at
          ...
          <p>
            ...
            point <Latex math="(\pi, \sqrt 2)" /> isn't.
          </p>
        </section>
        ...
        <section>
          ...
          <p>
            If you think that <Latex math="a^2 + b^2 = c^2" /> and{" "}
            <Latex math="x^2 + y^2 = 1" /> have a similar structure, then you
            would be right, and this is not by coincidence!
          </p>
          <p>
            Let's use this similar structure to prove that the set of primitive
            Pythagorean Triples and the set of rational points on the unit
            circle have the same cardinality by pairing them up.
          </p>
          <p>
            Let <Latex math="\{a, b, c\}" /> be a primitive Pythagorean Triple.
            We would like to pair it up with some unique rational point on the
            unit circle.
          </p>
          <p>
            Let <Latex math="x=\frac a c" /> and <Latex math="y=\frac b c" />.
          ...
          />
          <p>
            Thus, we know the point <Latex math="(x, y)" /> is actually on the
        ...
        <section>
          ...
          <p>
            ...
            y-intercept. In this case, we'll call our y-intercept{" "}
            <Latex math="t" /> so that <Latex math="b=t" /> here. Notice that if
            we go from the green point on the graph to our y-intercept, we
            travel 1 unit to the right and <Latex math="b" /> units up or down.
            Thus:
          </p>
          <Latex
            math="m = \text{Slope} = \frac{\text{Rise}}{\text{Run}} = \frac{t}{1} = t"
            center
          />
          <p>
            This means the equation for our black line is{" "}
            <Latex math="y=tx+t=t(1+x)" />.
          </p>
          <p>
            If we divide both sides of this equation by <Latex math="(1+x)" />,
            we will get:
          </p>
          <Latex math="t = \frac{y}{1+x}" center />
          <p>
            Thus, every rational point corresponds to a point on our circle.
            Also, while we won't do this here, some algebra will show that:
          </p>
          <Latex
            math="x=\frac{1 - t^2}{1 + t^2} \text{ and } y=\frac{2t}{1 + t^2}"
            center
          />
          <p>
            This allows us to get a unique rational point on the unit circle
          ...
          <p>
            ...
            rational points on the unit circle, and the set of primitive
            Pythagorean Triples all have the same size (up to finitely many
            exceptional points and symmetries which preserve cardinality). This
            is one example of why talking about rational points on curves is so
            powerful.
          </p>
        </section>
        <section>
          ...
          <p>
            Fermat's Last Theorem says that there are no three positive integers{" "}
            <Latex math="\{a, b, c\}" /> that satisfy the equation{" "}
            <Latex math="a^n + b^n = c^n" /> whenever <Latex math="n>2" /> is an
            integer. While Fermat's Last Theorem was first stated in the 1630s
            by Pierre de Fermat, it wasn't proven until the 1990s by Andrew
          ...
          <p>
            ...
            <Latex math="c^2" /> but with <Latex math="c^n" /> in this case, we
          ...
          <p>
            Using Fermat's Last Theorem, one can show that these curves have
            only the trivial rational points. That is, those which are labeled
            on the graphs above.
          </p>
        </section>
        <section>
          ...
          <p>
            Let <Latex math="x, y" /> be variables and let <Latex math="a, b" />{" "}
            be rational numbers. An <em>elliptic curve</em> is a smooth curve in
            2D space that is defined by an equation of the form:
          </p>
          <Latex math="y^2 = x^3 + ax + b" center />
          <h3>Groups</h3>
          <p>
            A <em>group</em> is a set of things which has an operation on them
          ...
          <p>
            For example, the integers are a group under the operation of
            addition.
          </p>
          <h3>Group Operation of Elliptic Curves</h3>
          <p>
            The rational points on an elliptic curve are a group under the
            following operation:
          </p>
          <ol>
            <li>
              Pick two points <Latex math="P \text{ and } Q" /> to add. These
              are the two blue points on the graph below.
            </li>
            <li>
              Draw a line through <Latex math="P \text{ and } Q" />.
            </li>
            <li>
              Find the third point of intersection of this line with the curve.
              Call it <Latex math="R" />. This is the purple circle on the graph
              below.
            </li>
            <li>
              Flip the point <Latex math="R" /> across the x-axis. This new
              point is <Latex math="P+Q" />. This final point is represented on
        ...
        <section>
          ...
          <p>
            We say a group is <em>finitely generated</em> if there is a finite
            set of elements in the group for which everything in the group can
            be expressed as a sum of those elements and their inverses with
            potential repetition. For example, the integers are finitely
            generated by the set <Latex math="\{1\}" />.
          </p>
          <p>
            The <em>Mordell-Weil Theorem</em> says that the rational points on
            an elliptic curve are finitely generated. This means that no matter
            what an elliptic curve looks like, you will always be able to find a{" "}
            <em>finite</em> set of points on it that generate every single
